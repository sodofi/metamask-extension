...
export const useMultichainBalances = () => {
  ...
  const assetsWithBalance = useMemo(() => {
    ...
    console.log(
      '==========makesure tokensWithBalance isnot constantly running',
    );
  ...
  const balanceByChainId = useMemo(() => {
    const balanceByChainId = assetsWithBalance.reduce(
      (acc: Record<`0x${string}`, number>, { chainId, tokenFiatAmount }) => {
        if (!acc[chainId]) {
          acc[chainId] = 0;
        }
        if (tokenFiatAmount) {
          acc[chainId] = acc[chainId] + tokenFiatAmount;
        }
        return acc;
      },
      {},
    ...
    console.log('=====makesure balanceByChainId isnot constantly running');
